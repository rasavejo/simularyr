...
For the moment, the cache model is only a stochastic one, based only on the cache miss rates which force us to know this rate before simulating, an information that may not be known. Also this model is not able to adjust when changing parameters such as the cache size. In real life, we expect a changing cache size to impact the result but here it will not since the probability is coded in the task description. We also don't simulate the hard drive accesses.
